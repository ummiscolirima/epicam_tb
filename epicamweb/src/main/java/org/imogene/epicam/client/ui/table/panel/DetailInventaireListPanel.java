...
import org.imogene.epicam.client.ui.table.DetailInventaireDynaTable;
import org.imogene.epicam.client.dataprovider.DetailInventaireDataProvider;
...
/**
 * Panel that contains the DetailInventaire Dynatable
 * @author Medes-IMPS
 */
public class DetailInventaireListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DetailInventaireListPanel> {
	}
	...
	@UiField(provided = true)
	DetailInventaireDynaTable listComposite;
	 ...
	 */
	public DetailInventaireListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().detailInventaire_table_text());
		if (EpicamIconConstants.DETAILINVENTAIRE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.DETAILINVENTAIRE_ICON);

		/* data provider */
		DetailInventaireDataProvider provider = new DetailInventaireDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteDetailInventaire()
				&& AccessManager.canEditDetailInventaire())
			listComposite = new DetailInventaireDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DetailInventaireDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DetailInventaire entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(DetailInventaireDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DetailInventaire entries
	 */
