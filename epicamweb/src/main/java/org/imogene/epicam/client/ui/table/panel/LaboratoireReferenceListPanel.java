...
import org.imogene.epicam.client.ui.table.LaboratoireReferenceDynaTable;
...
/**
 * Panel that contains the LaboratoireReference Dynatable
 * @author Medes-IMPS
 */
public class LaboratoireReferenceListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, LaboratoireReferenceListPanel> {
	}
	...
	@UiField(provided = true)
	LaboratoireReferenceDynaTable listComposite;
	 ...
	 */
	public LaboratoireReferenceListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel
				.setTitle(NLS.constants().laboratoireReference_table_text());
		if (EpicamIconConstants.LABORATOIREREFERENCE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.LABORATOIREREFERENCE_ICON);

		/* data provider */
		LaboratoireReferenceDataProvider provider = new LaboratoireReferenceDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteLaboratoireReference()
			...
			listComposite = new LaboratoireReferenceDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new LaboratoireReferenceDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists LaboratoireReference entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			LaboratoireReferenceDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists LaboratoireReference entries
	 */
