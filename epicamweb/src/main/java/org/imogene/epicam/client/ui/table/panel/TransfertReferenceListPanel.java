...
import org.imogene.epicam.client.ui.table.TransfertReferenceDynaTable;
import org.imogene.epicam.client.dataprovider.TransfertReferenceDataProvider;
...
/**
 * Panel that contains the TransfertReference Dynatable
 * @author Medes-IMPS
 */
public class TransfertReferenceListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, TransfertReferenceListPanel> {
	}
	...
	@UiField(provided = true)
	TransfertReferenceDynaTable listComposite;
	 ...
	 */
	public TransfertReferenceListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().transfertReference_table_text());
		if (EpicamIconConstants.TRANSFERTREFERENCE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.TRANSFERTREFERENCE_ICON);

		/* data provider */
		TransfertReferenceDataProvider provider = new TransfertReferenceDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteTransfertReference()
			...
			listComposite = new TransfertReferenceDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new TransfertReferenceDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists TransfertReference entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			TransfertReferenceDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists TransfertReference entries
	 */
