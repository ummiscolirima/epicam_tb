...
import org.imogene.epicam.client.ui.table.PriseMedicamenteuseDynaTable;
...
/**
 * Panel that contains the PriseMedicamenteuse Dynatable
 * @author Medes-IMPS
 */
public class PriseMedicamenteuseListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, PriseMedicamenteuseListPanel> {
	}
	...
	@UiField(provided = true)
	PriseMedicamenteuseDynaTable listComposite;
	 ...
	 */
	public PriseMedicamenteuseListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().priseMedicamenteuse_table_text());
		if (EpicamIconConstants.PRISEMEDICAMENTEUSE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.PRISEMEDICAMENTEUSE_ICON);

		/* data provider */
		PriseMedicamenteuseDataProvider provider = new PriseMedicamenteuseDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeletePriseMedicamenteuse()
			...
			listComposite = new PriseMedicamenteuseDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new PriseMedicamenteuseDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists PriseMedicamenteuse entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			PriseMedicamenteuseDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists PriseMedicamenteuse entries
	 */
