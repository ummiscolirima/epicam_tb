...
import org.imogene.epicam.client.ui.table.CommandeDynaTable;
...
/**
 * Panel that contains the Commande Dynatable
 * @author Medes-IMPS
 */
public class CommandeListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, CommandeListPanel> {
	}
	...
	@UiField(provided = true)
	CommandeDynaTable listComposite;
	 ...
	 */
	public CommandeListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().commande_table_text());
		if (EpicamIconConstants.COMMANDE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.COMMANDE_ICON);

		/* data provider */
		CommandeDataProvider provider = new CommandeDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteCommande()
			...
			listComposite = new CommandeDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new CommandeDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Commande entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(CommandeDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Commande entries
	 */
