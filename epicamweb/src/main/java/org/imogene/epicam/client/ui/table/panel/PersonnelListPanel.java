...
import org.imogene.epicam.client.ui.table.PersonnelDynaTable;
...
/**
 * Panel that contains the Personnel Dynatable
 * @author Medes-IMPS
 */
public class PersonnelListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, PersonnelListPanel> {
	}
	...
	@UiField(provided = true)
	PersonnelDynaTable listComposite;
	 ...
	 */
	public PersonnelListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().personnel_table_text());
		if (EpicamIconConstants.PERSONNEL_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.PERSONNEL_ICON);

		/* data provider */
		PersonnelDataProvider provider = new PersonnelDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeletePersonnel()
			...
			listComposite = new PersonnelDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new PersonnelDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Personnel entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(PersonnelDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Personnel entries
	 */
