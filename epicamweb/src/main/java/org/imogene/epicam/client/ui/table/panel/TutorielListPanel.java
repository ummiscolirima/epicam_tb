...
import org.imogene.epicam.client.ui.table.TutorielDynaTable;
import org.imogene.epicam.client.dataprovider.TutorielDataProvider;
...
/**
 * Panel that contains the Tutoriel Dynatable
 * @author Medes-IMPS
 */
public class TutorielListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, TutorielListPanel> {
	}
	...
	@UiField(provided = true)
	TutorielDynaTable listComposite;
	 ...
	 */
	public TutorielListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().tutoriel_table_text());
		if (EpicamIconConstants.TUTORIEL_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.TUTORIEL_ICON);

		/* data provider */
		TutorielDataProvider provider = new TutorielDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteTutoriel()
			...
			listComposite = new TutorielDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new TutorielDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Tutoriel entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(TutorielDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Tutoriel entries
	 */
