...
import org.imogene.epicam.client.ui.table.RegionDynaTable;
...
/**
 * Panel that contains the Region Dynatable
 * @author Medes-IMPS
 */
public class RegionListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, RegionListPanel> {
	}
	 ...
	 */
	public RegionListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().region_table_text());
		if (EpicamIconConstants.REGION_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.REGION_ICON);

		/* data provider */
		RegionDataProvider provider = new RegionDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteRegion() && AccessManager.canEditRegion())
			listComposite = new RegionDynaTable(requestFactory, provider, true);
		else
			listComposite = new RegionDynaTable(requestFactory, provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Region entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(RegionDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Region entries
	 */
