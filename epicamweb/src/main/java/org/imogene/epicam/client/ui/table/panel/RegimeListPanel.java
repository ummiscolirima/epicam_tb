...
import org.imogene.epicam.client.ui.table.RegimeDynaTable;
...
/**
 * Panel that contains the Regime Dynatable
 * @author Medes-IMPS
 */
public class RegimeListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, RegimeListPanel> {
	}
	 ...
	 */
	public RegimeListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().regime_table_text());
		if (EpicamIconConstants.REGIME_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.REGIME_ICON);

		/* data provider */
		RegimeDataProvider provider = new RegimeDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteRegime() && AccessManager.canEditRegime())
			listComposite = new RegimeDynaTable(requestFactory, provider, true);
		else
			listComposite = new RegimeDynaTable(requestFactory, provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Regime entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(RegimeDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Regime entries
	 */
