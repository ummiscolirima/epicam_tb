...
import org.imogene.epicam.client.ui.table.ArriveePersonnelDynaTable;
import org.imogene.epicam.client.dataprovider.ArriveePersonnelDataProvider;
...
/**
 * Panel that contains the ArriveePersonnel Dynatable
 * @author Medes-IMPS
 */
public class ArriveePersonnelListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ArriveePersonnelListPanel> {
	}
	...
	@UiField(provided = true)
	ArriveePersonnelDynaTable listComposite;
	 ...
	 */
	public ArriveePersonnelListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().arriveePersonnel_table_text());
		if (EpicamIconConstants.ARRIVEEPERSONNEL_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.ARRIVEEPERSONNEL_ICON);

		/* data provider */
		ArriveePersonnelDataProvider provider = new ArriveePersonnelDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteArriveePersonnel()
				&& AccessManager.canEditArriveePersonnel())
			listComposite = new ArriveePersonnelDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new ArriveePersonnelDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists ArriveePersonnel entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(ArriveePersonnelDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists ArriveePersonnel entries
	 */
