...
import org.imogene.epicam.client.ui.table.HorsUsageDynaTable;
import org.imogene.epicam.client.dataprovider.HorsUsageDataProvider;
...
/**
 * Panel that contains the HorsUsage Dynatable
 * @author Medes-IMPS
 */
public class HorsUsageListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, HorsUsageListPanel> {
	}
	...
	@UiField(provided = true)
	HorsUsageDynaTable listComposite;
	 ...
	 */
	public HorsUsageListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().horsUsage_table_text());
		if (EpicamIconConstants.HORSUSAGE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.HORSUSAGE_ICON);

		/* data provider */
		HorsUsageDataProvider provider = new HorsUsageDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteHorsUsage()
			...
			listComposite = new HorsUsageDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new HorsUsageDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists HorsUsage entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(HorsUsageDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists HorsUsage entries
	 */
