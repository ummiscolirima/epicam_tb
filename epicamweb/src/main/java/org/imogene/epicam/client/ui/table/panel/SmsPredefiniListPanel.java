...
import org.imogene.epicam.client.ui.table.SmsPredefiniDynaTable;
...
/**
 * Panel that contains the SmsPredefini Dynatable
 * @author Medes-IMPS
 */
public class SmsPredefiniListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, SmsPredefiniListPanel> {
	}
	...
	@UiField(provided = true)
	SmsPredefiniDynaTable listComposite;
	 ...
	 */
	public SmsPredefiniListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().smsPredefini_table_text());
		if (EpicamIconConstants.SMSPREDEFINI_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.SMSPREDEFINI_ICON);

		/* data provider */
		SmsPredefiniDataProvider provider = new SmsPredefiniDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteSmsPredefini()
			...
			listComposite = new SmsPredefiniDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new SmsPredefiniDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists SmsPredefini entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(SmsPredefiniDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists SmsPredefini entries
	 */
