...
import org.imogene.epicam.client.ui.table.LotDynaTable;
...
/**
 * Panel that contains the Lot Dynatable
 * @author Medes-IMPS
 */
public class LotListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, LotListPanel> {
	}
	 ...
	 */
	public LotListPanel(EpicamRequestFactory requestFactory, String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().lot_table_text());
		if (EpicamIconConstants.LOT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.LOT_ICON);

		/* data provider */
		LotDataProvider provider = new LotDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteLot() && AccessManager.canEditLot())
			listComposite = new LotDynaTable(requestFactory, provider, true);
		else
			listComposite = new LotDynaTable(requestFactory, provider, false);
	 ...
	 * @param requestFactory
	 */
	public LotListPanel(EpicamRequestFactory requestFactory) {
		this(requestFactory, null);
	}

	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Lot entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(LotDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Lot entries
	 */
