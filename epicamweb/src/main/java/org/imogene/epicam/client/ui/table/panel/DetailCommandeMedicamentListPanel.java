...
import org.imogene.epicam.client.ui.table.DetailCommandeMedicamentDynaTable;
...
/**
 * Panel that contains the DetailCommandeMedicament Dynatable
 * @author Medes-IMPS
 */
public class DetailCommandeMedicamentListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder
			extends
				UiBinder<Widget, DetailCommandeMedicamentListPanel> {
	}
	...
	@UiField(provided = true)
	DetailCommandeMedicamentDynaTable listComposite;
	 ...
	 */
	public DetailCommandeMedicamentListPanel(
			EpicamRequestFactory requestFactory, String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants()
				.detailCommandeMedicament_table_text());
		if (EpicamIconConstants.DETAILCOMMANDEMEDICAMENT_ICON != null)
			wrapperPanel
					.setIcon(EpicamIconConstants.DETAILCOMMANDEMEDICAMENT_ICON);

		/* data provider */
		DetailCommandeMedicamentDataProvider provider = new DetailCommandeMedicamentDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteDetailCommandeMedicament()
			...
			listComposite = new DetailCommandeMedicamentDynaTable(
					requestFactory, provider, true);
		else
			listComposite = new DetailCommandeMedicamentDynaTable(
					requestFactory, provider, false);
	 ...
	 * @param requestFactory
	 */
	public DetailCommandeMedicamentListPanel(EpicamRequestFactory requestFactory) {
		this(requestFactory, null);
	}

	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DetailCommandeMedicament entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			DetailCommandeMedicamentDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DetailCommandeMedicament entries
	 */
	...
	private void setListActions() {
		...
		if (AccessManager.canDirectAccessExamenBiologique()
			...
			SimpleMenuItem item = new SimpleMenuItem(
					requestFactory.getEventBus(), NLS.constants()
							.examenBiologique_name_plur(), command);
			listButton.addPopupPanelContent(item);
		}
