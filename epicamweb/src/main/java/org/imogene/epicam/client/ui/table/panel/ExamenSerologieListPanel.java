...
import org.imogene.epicam.client.ui.table.ExamenSerologieDynaTable;
...
/**
 * Panel that contains the ExamenSerologie Dynatable
 * @author Medes-IMPS
 */
public class ExamenSerologieListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ExamenSerologieListPanel> {
	}
	...
	@UiField(provided = true)
	ExamenSerologieDynaTable listComposite;
	 ...
	 */
	public ExamenSerologieListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().examenSerologie_table_text());
		if (EpicamIconConstants.EXAMENSEROLOGIE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.EXAMENSEROLOGIE_ICON);

		/* data provider */
		ExamenSerologieDataProvider provider = new ExamenSerologieDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteExamenSerologie()
			...
			listComposite = new ExamenSerologieDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new ExamenSerologieDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists ExamenSerologie entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(ExamenSerologieDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists ExamenSerologie entries
	 */
