...
import org.imogene.epicam.client.ui.table.DetailRavitaillementDynaTable;
...
/**
 * Panel that contains the DetailRavitaillement Dynatable
 * @author Medes-IMPS
 */
public class DetailRavitaillementListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DetailRavitaillementListPanel> {
	}
	...
	@UiField(provided = true)
	DetailRavitaillementDynaTable listComposite;
	 ...
	 */
	public DetailRavitaillementListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel
				.setTitle(NLS.constants().detailRavitaillement_table_text());
		if (EpicamIconConstants.DETAILRAVITAILLEMENT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.DETAILRAVITAILLEMENT_ICON);

		/* data provider */
		DetailRavitaillementDataProvider provider = new DetailRavitaillementDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteDetailRavitaillement()
			...
			listComposite = new DetailRavitaillementDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DetailRavitaillementDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DetailRavitaillement entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			DetailRavitaillementDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DetailRavitaillement entries
	 */
