...
import org.imogene.epicam.client.ui.table.DetailReceptionIntrantDynaTable;
...
/**
 * Panel that contains the DetailReceptionIntrant Dynatable
 * @author Medes-IMPS
 */
public class DetailReceptionIntrantListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DetailReceptionIntrantListPanel> {
	}
	...
	@UiField(provided = true)
	DetailReceptionIntrantDynaTable listComposite;
	 ...
	 */
	public DetailReceptionIntrantListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants()
				.detailReceptionIntrant_table_text());
		if (EpicamIconConstants.DETAILRECEPTIONINTRANT_ICON != null)
			wrapperPanel
					.setIcon(EpicamIconConstants.DETAILRECEPTIONINTRANT_ICON);

		/* data provider */
		DetailReceptionIntrantDataProvider provider = new DetailReceptionIntrantDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteDetailReceptionIntrant()
				&& AccessManager.canEditDetailReceptionIntrant())
			listComposite = new DetailReceptionIntrantDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DetailReceptionIntrantDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DetailReceptionIntrant entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			DetailReceptionIntrantDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DetailReceptionIntrant entries
	 */
