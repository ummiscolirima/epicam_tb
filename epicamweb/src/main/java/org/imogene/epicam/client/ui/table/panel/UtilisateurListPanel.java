...
import org.imogene.epicam.client.ui.table.UtilisateurDynaTable;
import org.imogene.epicam.client.dataprovider.UtilisateurDataProvider;
...
/**
 * Panel that contains the Utilisateur Dynatable
 * @author Medes-IMPS
 */
public class UtilisateurListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, UtilisateurListPanel> {
	}
	...
	@UiField(provided = true)
	UtilisateurDynaTable listComposite;
	 ...
	 */
	public UtilisateurListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().utilisateur_table_text());
		if (EpicamIconConstants.UTILISATEUR_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.UTILISATEUR_ICON);

		/* data provider */
		UtilisateurDataProvider provider = new UtilisateurDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteUtilisateur()
			...
			listComposite = new UtilisateurDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new UtilisateurDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Utilisateur entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(UtilisateurDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Utilisateur entries
	 */
