...
import org.imogene.epicam.client.ui.table.PriseMedicamentRegimeDynaTable;
...
/**
 * Panel that contains the PriseMedicamentRegime Dynatable
 * @author Medes-IMPS
 */
public class PriseMedicamentRegimeListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, PriseMedicamentRegimeListPanel> {
	}
	...
	@UiField(provided = true)
	PriseMedicamentRegimeDynaTable listComposite;
	 ...
	 */
	public PriseMedicamentRegimeListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants()
				.priseMedicamentRegime_table_text());
		if (EpicamIconConstants.PRISEMEDICAMENTREGIME_ICON != null)
			wrapperPanel
					.setIcon(EpicamIconConstants.PRISEMEDICAMENTREGIME_ICON);

		/* data provider */
		PriseMedicamentRegimeDataProvider provider = new PriseMedicamentRegimeDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeletePriseMedicamentRegime()
			...
			listComposite = new PriseMedicamentRegimeDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new PriseMedicamentRegimeDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists PriseMedicamentRegime entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			PriseMedicamentRegimeDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists PriseMedicamentRegime entries
	 */
