...
import org.imogene.epicam.client.ui.table.InventaireDynaTable;
import org.imogene.epicam.client.dataprovider.InventaireDataProvider;
...
/**
 * Panel that contains the Inventaire Dynatable
 * @author Medes-IMPS
 */
public class InventaireListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, InventaireListPanel> {
	}
	...
	@UiField(provided = true)
	InventaireDynaTable listComposite;
	 ...
	 */
	public InventaireListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().inventaire_table_text());
		if (EpicamIconConstants.INVENTAIRE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.INVENTAIRE_ICON);

		/* data provider */
		InventaireDataProvider provider = new InventaireDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteInventaire()
			...
			listComposite = new InventaireDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new InventaireDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Inventaire entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(InventaireDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Inventaire entries
	 */
