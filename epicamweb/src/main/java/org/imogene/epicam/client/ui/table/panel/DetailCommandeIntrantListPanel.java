...
import org.imogene.epicam.client.ui.table.DetailCommandeIntrantDynaTable;
...
/**
 * Panel that contains the DetailCommandeIntrant Dynatable
 * @author Medes-IMPS
 */
public class DetailCommandeIntrantListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DetailCommandeIntrantListPanel> {
	}
	...
	@UiField(provided = true)
	DetailCommandeIntrantDynaTable listComposite;
	 ...
	 */
	public DetailCommandeIntrantListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants()
				.detailCommandeIntrant_table_text());
		if (EpicamIconConstants.DETAILCOMMANDEINTRANT_ICON != null)
			wrapperPanel
					.setIcon(EpicamIconConstants.DETAILCOMMANDEINTRANT_ICON);

		/* data provider */
		DetailCommandeIntrantDataProvider provider = new DetailCommandeIntrantDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteDetailCommandeIntrant()
			...
			listComposite = new DetailCommandeIntrantDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DetailCommandeIntrantDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DetailCommandeIntrant entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			DetailCommandeIntrantDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DetailCommandeIntrant entries
	 */
