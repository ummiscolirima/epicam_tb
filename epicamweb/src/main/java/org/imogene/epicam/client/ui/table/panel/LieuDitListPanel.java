...
import org.imogene.epicam.client.ui.table.LieuDitDynaTable;
...
/**
 * Panel that contains the LieuDit Dynatable
 * @author Medes-IMPS
 */
public class LieuDitListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, LieuDitListPanel> {
	}
	...
	@UiField(provided = true)
	LieuDitDynaTable listComposite;
	 ...
	 */
	public LieuDitListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().lieuDit_table_text());
		if (EpicamIconConstants.LIEUDIT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.LIEUDIT_ICON);

		/* data provider */
		LieuDitDataProvider provider = new LieuDitDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteLieuDit() && AccessManager.canEditLieuDit())
			listComposite = new LieuDitDynaTable(requestFactory, provider, true);
		else
			listComposite = new LieuDitDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists LieuDit entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(LieuDitDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists LieuDit entries
	 */
