...
import org.imogene.epicam.client.ui.table.CandidatureFormationDynaTable;
...
/**
 * Panel that contains the CandidatureFormation Dynatable
 * @author Medes-IMPS
 */
public class CandidatureFormationListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, CandidatureFormationListPanel> {
	}
	...
	@UiField(provided = true)
	CandidatureFormationDynaTable listComposite;
	 ...
	 */
	public CandidatureFormationListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel
				.setTitle(NLS.constants().candidatureFormation_table_text());
		if (EpicamIconConstants.CANDIDATUREFORMATION_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.CANDIDATUREFORMATION_ICON);

		/* data provider */
		CandidatureFormationDataProvider provider = new CandidatureFormationDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteCandidatureFormation()
			...
			listComposite = new CandidatureFormationDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new CandidatureFormationDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists CandidatureFormation entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			CandidatureFormationDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists CandidatureFormation entries
	 */
