...
import org.imogene.epicam.client.ui.table.CasTuberculoseDynaTable;
...
/**
 * Panel that contains the CasTuberculose Dynatable
 * @author Medes-IMPS
 */
public class CasTuberculoseListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, CasTuberculoseListPanel> {
	}
	...
	@UiField(provided = true)
	CasTuberculoseDynaTable listComposite;
	 ...
	 */
	public CasTuberculoseListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().casTuberculose_table_text());
		if (EpicamIconConstants.CASTUBERCULOSE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.CASTUBERCULOSE_ICON);

		/* data provider */
		CasTuberculoseDataProvider provider = new CasTuberculoseDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteCasTuberculose()
			...
			listComposite = new CasTuberculoseDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new CasTuberculoseDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists CasTuberculose entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(CasTuberculoseDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists CasTuberculose entries
	 */
