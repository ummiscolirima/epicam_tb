...
import org.imogene.epicam.client.ui.table.FormationDynaTable;
...
/**
 * Panel that contains the Formation Dynatable
 * @author Medes-IMPS
 */
public class FormationListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, FormationListPanel> {
	}
	...
	@UiField(provided = true)
	FormationDynaTable listComposite;
	 ...
	 */
	public FormationListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().formation_table_text());
		if (EpicamIconConstants.FORMATION_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.FORMATION_ICON);

		/* data provider */
		FormationDataProvider provider = new FormationDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteFormation()
			...
			listComposite = new FormationDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new FormationDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Formation entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(FormationDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Formation entries
	 */
