...
import org.imogene.epicam.client.ui.table.ExamenATBDynaTable;
...
/**
 * Panel that contains the ExamenATB Dynatable
 * @author Medes-IMPS
 */
public class ExamenATBListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ExamenATBListPanel> {
	}
	...
	@UiField(provided = true)
	ExamenATBDynaTable listComposite;
	 ...
	 */
	public ExamenATBListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().examenATB_table_text());
		if (EpicamIconConstants.EXAMENATB_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.EXAMENATB_ICON);

		/* data provider */
		ExamenATBDataProvider provider = new ExamenATBDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteExamenATB()
				&& AccessManager.canEditExamenATB())
			listComposite = new ExamenATBDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new ExamenATBDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists ExamenATB entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(ExamenATBDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists ExamenATB entries
	 */
