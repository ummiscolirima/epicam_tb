...
import org.imogene.epicam.client.ui.table.DistrictSanteDynaTable;
...
/**
 * Panel that contains the DistrictSante Dynatable
 * @author Medes-IMPS
 */
public class DistrictSanteListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DistrictSanteListPanel> {
	}
	...
	@UiField(provided = true)
	DistrictSanteDynaTable listComposite;
	 ...
	 */
	public DistrictSanteListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().districtSante_table_text());
		if (EpicamIconConstants.DISTRICTSANTE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.DISTRICTSANTE_ICON);

		/* data provider */
		DistrictSanteDataProvider provider = new DistrictSanteDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteDistrictSante()
			...
			listComposite = new DistrictSanteDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DistrictSanteDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DistrictSante entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(DistrictSanteDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DistrictSante entries
	 */
