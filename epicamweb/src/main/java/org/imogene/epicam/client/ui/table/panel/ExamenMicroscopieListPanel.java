...
import org.imogene.epicam.client.ui.table.ExamenMicroscopieDynaTable;
...
/**
 * Panel that contains the ExamenMicroscopie Dynatable
 * @author Medes-IMPS
 */
public class ExamenMicroscopieListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ExamenMicroscopieListPanel> {
	}
	...
	@UiField(provided = true)
	ExamenMicroscopieDynaTable listComposite;
	 ...
	 */
	public ExamenMicroscopieListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().examenMicroscopie_table_text());
		if (EpicamIconConstants.EXAMENMICROSCOPIE_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.EXAMENMICROSCOPIE_ICON);

		/* data provider */
		ExamenMicroscopieDataProvider provider = new ExamenMicroscopieDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteExamenMicroscopie()
			...
			listComposite = new ExamenMicroscopieDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new ExamenMicroscopieDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists ExamenMicroscopie entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(
			ExamenMicroscopieDataProvider provider, String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists ExamenMicroscopie entries
	 */
