...
import org.imogene.epicam.client.ui.table.MedicamentDynaTable;
...
/**
 * Panel that contains the Medicament Dynatable
 * @author Medes-IMPS
 */
public class MedicamentListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, MedicamentListPanel> {
	}
	...
	@UiField(provided = true)
	MedicamentDynaTable listComposite;
	 ...
	 */
	public MedicamentListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().medicament_table_text());
		if (EpicamIconConstants.MEDICAMENT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.MEDICAMENT_ICON);

		/* data provider */
		MedicamentDataProvider provider = new MedicamentDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteMedicament()
			...
			listComposite = new MedicamentDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new MedicamentDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Medicament entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(MedicamentDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Medicament entries
	 */
