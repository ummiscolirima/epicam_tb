...
import org.imogene.epicam.client.ui.table.IntrantDynaTable;
...
/**
 * Panel that contains the Intrant Dynatable
 * @author Medes-IMPS
 */
public class IntrantListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, IntrantListPanel> {
	}
	...
	@UiField(provided = true)
	IntrantDynaTable listComposite;
	 ...
	 */
	public IntrantListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().intrant_table_text());
		if (EpicamIconConstants.INTRANT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.INTRANT_ICON);

		/* data provider */
		IntrantDataProvider provider = new IntrantDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteIntrant() && AccessManager.canEditIntrant())
			listComposite = new IntrantDynaTable(requestFactory, provider, true);
		else
			listComposite = new IntrantDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Intrant entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(IntrantDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Intrant entries
	 */
