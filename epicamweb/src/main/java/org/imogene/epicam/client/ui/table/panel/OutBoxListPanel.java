...
import org.imogene.epicam.client.ui.table.OutBoxDynaTable;
import org.imogene.epicam.client.dataprovider.OutBoxDataProvider;
...
/**
 * Panel that contains the OutBox Dynatable
 * @author Medes-IMPS
 */
public class OutBoxListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, OutBoxListPanel> {
	}
	 ...
	 */
	public OutBoxListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().outBox_table_text());
		if (EpicamIconConstants.OUTBOX_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.OUTBOX_ICON);

		/* data provider */
		OutBoxDataProvider provider = new OutBoxDataProvider(requestFactory);
		...
		if (AccessManager.canDeleteOutBox() && AccessManager.canEditOutBox())
			listComposite = new OutBoxDynaTable(requestFactory, provider, true);
		else
			listComposite = new OutBoxDynaTable(requestFactory, provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists OutBox entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(OutBoxDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists OutBox entries
	 */
