...
import org.imogene.epicam.client.ui.table.DepartPersonnelDynaTable;
import org.imogene.epicam.client.dataprovider.DepartPersonnelDataProvider;
...
/**
 * Panel that contains the DepartPersonnel Dynatable
 * @author Medes-IMPS
 */
public class DepartPersonnelListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, DepartPersonnelListPanel> {
	}
	...
	@UiField(provided = true)
	DepartPersonnelDynaTable listComposite;
	 ...
	 */
	public DepartPersonnelListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().departPersonnel_table_text());
		if (EpicamIconConstants.DEPARTPERSONNEL_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.DEPARTPERSONNEL_ICON);

		/* data provider */
		DepartPersonnelDataProvider provider = new DepartPersonnelDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteDepartPersonnel()
			...
			listComposite = new DepartPersonnelDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new DepartPersonnelDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists DepartPersonnel entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(DepartPersonnelDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists DepartPersonnel entries
	 */
