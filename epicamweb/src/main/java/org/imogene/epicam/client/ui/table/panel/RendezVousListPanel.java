...
import org.imogene.epicam.client.ui.table.RendezVousDynaTable;
...
/**
 * Panel that contains the RendezVous Dynatable
 * @author Medes-IMPS
 */
public class RendezVousListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, RendezVousListPanel> {
	}
	...
	@UiField(provided = true)
	RendezVousDynaTable listComposite;
	 ...
	 */
	public RendezVousListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().rendezVous_table_text());
		if (EpicamIconConstants.RENDEZVOUS_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.RENDEZVOUS_ICON);

		/* data provider */
		RendezVousDataProvider provider = new RendezVousDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteRendezVous()
			...
			listComposite = new RendezVousDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new RendezVousDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists RendezVous entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(RendezVousDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists RendezVous entries
	 */
