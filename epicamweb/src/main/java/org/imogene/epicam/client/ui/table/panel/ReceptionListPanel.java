...
import org.imogene.epicam.client.ui.table.ReceptionDynaTable;
...
/**
 * Panel that contains the Reception Dynatable
 * @author Medes-IMPS
 */
public class ReceptionListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, ReceptionListPanel> {
	}
	...
	@UiField(provided = true)
	ReceptionDynaTable listComposite;
	 ...
	 */
	public ReceptionListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().reception_table_text());
		if (EpicamIconConstants.RECEPTION_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.RECEPTION_ICON);

		/* data provider */
		ReceptionDataProvider provider = new ReceptionDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteReception()
				&& AccessManager.canEditReception())
			listComposite = new ReceptionDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new ReceptionDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Reception entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(ReceptionDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Reception entries
	 */
