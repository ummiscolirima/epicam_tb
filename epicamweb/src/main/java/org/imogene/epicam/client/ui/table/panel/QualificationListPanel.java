...
import org.imogene.epicam.client.ui.table.QualificationDynaTable;
...
/**
 * Panel that contains the Qualification Dynatable
 * @author Medes-IMPS
 */
public class QualificationListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, QualificationListPanel> {
	}
	...
	@UiField(provided = true)
	QualificationDynaTable listComposite;
	 ...
	 */
	public QualificationListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().qualification_table_text());
		if (EpicamIconConstants.QUALIFICATION_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.QUALIFICATION_ICON);

		/* data provider */
		QualificationDataProvider provider = new QualificationDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteQualification()
				&& AccessManager.canEditQualification())
			listComposite = new QualificationDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new QualificationDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Qualification entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(QualificationDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Qualification entries
	 */
