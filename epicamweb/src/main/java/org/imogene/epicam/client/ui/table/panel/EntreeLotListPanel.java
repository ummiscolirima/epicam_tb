...
import org.imogene.epicam.client.ui.table.EntreeLotDynaTable;
...
/**
 * Panel that contains the EntreeLot Dynatable
 * @author Medes-IMPS
 */
public class EntreeLotListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, EntreeLotListPanel> {
	}
	...
	@UiField(provided = true)
	EntreeLotDynaTable listComposite;
	 ...
	 */
	public EntreeLotListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().entreeLot_table_text());
		if (EpicamIconConstants.ENTREELOT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.ENTREELOT_ICON);

		/* data provider */
		EntreeLotDataProvider provider = new EntreeLotDataProvider(
				requestFactory);
		...
		if (AccessManager.canDeleteEntreeLot()
			...
			listComposite = new EntreeLotDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new EntreeLotDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists EntreeLot entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(EntreeLotDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists EntreeLot entries
	 */
