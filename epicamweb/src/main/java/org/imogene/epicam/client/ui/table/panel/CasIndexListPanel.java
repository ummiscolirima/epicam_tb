...
import org.imogene.epicam.client.ui.table.CasIndexDynaTable;
...
/**
 * Panel that contains the CasIndex Dynatable
 * @author Medes-IMPS
 */
public class CasIndexListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, CasIndexListPanel> {
	}
	...
	@UiField(provided = true)
	CasIndexDynaTable listComposite;
	 ...
	 */
	public CasIndexListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		if (AccessManager.canDeleteCasIndex()
			...
			listComposite = new CasIndexDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new CasIndexDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists CasIndex entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(CasIndexDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists CasIndex entries
	 */
