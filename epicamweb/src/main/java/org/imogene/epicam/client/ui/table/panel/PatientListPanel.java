...
import org.imogene.epicam.client.ui.table.PatientDynaTable;
...
/**
 * Panel that contains the Patient Dynatable
 * @author Medes-IMPS
 */
public class PatientListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, PatientListPanel> {
	}
	...
	@UiField(provided = true)
	PatientDynaTable listComposite;
	 ...
	 */
	public PatientListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().patient_table_text());
		if (EpicamIconConstants.PATIENT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.PATIENT_ICON);

		/* data provider */
		PatientDataProvider provider = new PatientDataProvider(requestFactory);
		...
		if (AccessManager.canDeletePatient() && AccessManager.canEditPatient())
			listComposite = new PatientDynaTable(requestFactory, provider, true);
		else
			listComposite = new PatientDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Patient entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(PatientDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Patient entries
	 */
