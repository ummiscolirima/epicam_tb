...
import org.imogene.epicam.client.ui.table.SortieLotDynaTable;
...
/**
 * Panel that contains the SortieLot Dynatable
 * @author Medes-IMPS
 */
public class SortieLotListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, SortieLotListPanel> {
	}
	...
	@UiField(provided = true)
	SortieLotDynaTable listComposite;
	 ...
	 */
	public SortieLotListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().sortieLot_table_text());
		if (EpicamIconConstants.SORTIELOT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.SORTIELOT_ICON);

		/* data provider */
		SortieLotDataProvider provider = new SortieLotDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteSortieLot()
				&& AccessManager.canEditSortieLot())
			listComposite = new SortieLotDynaTable(requestFactory, provider,
					true);
		else
			listComposite = new SortieLotDynaTable(requestFactory, provider,
					false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists SortieLot entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(SortieLotDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists SortieLot entries
	 */
