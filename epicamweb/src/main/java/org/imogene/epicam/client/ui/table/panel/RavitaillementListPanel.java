...
import org.imogene.epicam.client.ui.table.RavitaillementDynaTable;
...
/**
 * Panel that contains the Ravitaillement Dynatable
 * @author Medes-IMPS
 */
public class RavitaillementListPanel extends Composite {

	private static final Binder uiBinder = GWT.create(Binder.class);

	interface Binder extends UiBinder<Widget, RavitaillementListPanel> {
	}
	...
	@UiField(provided = true)
	RavitaillementDynaTable listComposite;
	 ...
	 */
	public RavitaillementListPanel(EpicamRequestFactory requestFactory,
			String searchText) {
		...
		wrapperPanel.setTitle(NLS.constants().ravitaillement_table_text());
		if (EpicamIconConstants.RAVITAILLEMENT_ICON != null)
			wrapperPanel.setIcon(EpicamIconConstants.RAVITAILLEMENT_ICON);

		/* data provider */
		RavitaillementDataProvider provider = new RavitaillementDataProvider(
				requestFactory);
		...
		/* dynatable */
		if (AccessManager.canDeleteRavitaillement()
				&& AccessManager.canEditRavitaillement())
			listComposite = new RavitaillementDynaTable(requestFactory,
					provider, true);
		else
			listComposite = new RavitaillementDynaTable(requestFactory,
					provider, false);
	...
	/**
	 * Configures the Data provider that enables to get the data to feed the table that lists Ravitaillement entries
	 * @param provider
	 * @param searchText
	 * @return
	 */
	private String configureDataProvider(RavitaillementDataProvider provider,
			String searchText) {
	...
	/**
	 * Configures the Wrapper panel to manage the actions that are related to the table that lists Ravitaillement entries
	 */
