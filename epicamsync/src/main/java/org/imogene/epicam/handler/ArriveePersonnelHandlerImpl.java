...
import org.imogene.epicam.domain.dao.ArriveePersonnelDao;
...
/**
 * Implements a data handler for the ArriveePersonnel 
 * @author Medes-IMPS
 */
public class ArriveePersonnelHandlerImpl
		extends
			ImogBeanHandlerImpl<ArriveePersonnel>
		implements
			ArriveePersonnelHandler {

	@Autowired
	@Qualifier(value = "arriveePersonnelDao")
	private ArriveePersonnelDao dao;

	@Override
	public ArriveePersonnel createNewEntity(String id) {
		//TODO handle  with not null constraint values
		ArriveePersonnel entity = new ArriveePersonnel();
		entity.setId(id);
	...
	@Override
	protected void saveOrUpdate(ArriveePersonnel entity, boolean neu) {
	...
	@Override
	protected ArriveePersonnelDao getDao() {
		return dao;
	}
