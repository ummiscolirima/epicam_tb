...
import org.imogene.epicam.domain.dao.DetailRavitaillementDao;
...
/**
 * Implements a data handler for the DetailRavitaillement 
 * @author Medes-IMPS
 */
public class DetailRavitaillementHandlerImpl
		extends
			ImogBeanHandlerImpl<DetailRavitaillement>
		implements
			DetailRavitaillementHandler {

	@Autowired
	@Qualifier(value = "detailRavitaillementDao")
	private DetailRavitaillementDao dao;

	@Override
	public DetailRavitaillement createNewEntity(String id) {
		//TODO handle  with not null constraint values
		DetailRavitaillement entity = new DetailRavitaillement();
		entity.setId(id);
	...
	@Override
	protected void saveOrUpdate(DetailRavitaillement entity, boolean neu) {
	...
	@Override
	protected DetailRavitaillementDao getDao() {
		return dao;
	}
